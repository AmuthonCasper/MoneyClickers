...
function Start () {
    /*
    if (PlayerPrefs.HasKey("Savedupgrade3Cost")) { // Check if "Savedupgrade3Cost" is in the system
        ScoreCount.upgrade3Cost = PlayerPrefs.GetInt("Savedupgrade3Cost"); // Putting value of key onto variable
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.upgrade3Cost.ToString()); // Update upgrade3Cost text
    }
    else                                                     // if "Savedupgrade3Cost" isn't in the system
        PlayerPrefs.SetInt("Savedupgrade3Cost", 15000);  // Set "Savedupgrade3Cost" to standard (1000)
        ScoreCount.upgrade3Cost = PlayerPrefs.GetInt("Savedupgrade3Cost"); // Putting value of key onto variable
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.upgrade3Cost.ToString());  // Update upgrade3Cost text
}
...
function BuyUpgrade3 () {
    if (ScoreCount.money >= ScoreCount.upgrade3Cost) { // If you have enough money
        canPlaySound = true;
        ScoreCount.money -= ScoreCount.upgrade3Cost; // deduct money by the cost of the upgrade
        ScoreCount.mPerClick += 6; // money per click gains by 1
        ScoreCount.upgrade3Cost *= 1.70; // Upgrade3 goes up in price  
        textField.text = "Buy For " + ToDot2(ScoreCount.upgrade3Cost.ToString()); // Text updates to new price
    }
    else {
...
function NotEnough () {
    ...
    textField.text = "Buy For " + ToDot2(ScoreCount.upgrade3Cost.ToString()); // text returns normal price
    isNotEnough = false;
}
