...
function Start () {
    /*
    if (PlayerPrefs.HasKey("SavedautoUpgrade1Cost")) { // Check if "SavedautoUpgrade1Cost" is in the system 
        ScoreCount.autoUpgrade1Cost = PlayerPrefs.GetInt("SavedautoUpgrade1Cost"); // Putting value of key onto variable
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade1Cost.ToString()); // Update autoUpgrade1Cost text
    }
    else {                                                 // if "SavedautoUpgrade1Cost" isn't in the system
        PlayerPrefs.SetInt("SavedautoUpgrade1Cost", 10000);  // Set "SavedautoUpgrade1Cost" to standard (1000)
        ScoreCount.autoUpgrade1Cost = PlayerPrefs.GetInt("SavedautoUpgrade1Cost"); // Putting value of key onto variable
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade1Cost.ToString());  // Update autoUpgrade1Cost text
    }
...
function Update () {
    ...
    //PlayerPrefs.SetInt("SavedautoUpgrade1Cost", ScoreCount.autoUpgrade1Cost); // Putting value into key, every frame (Always saving)
}

function BuyAutoUpgrade1 () {
    if (ScoreCount.money >= ScoreCount.autoUpgrade1Cost) { // If you have enough money
        canPlaySound = true;
        ScoreCount.money -= ScoreCount.autoUpgrade1Cost; // deduct money by the cost of the upgrade
        ScoreCount.mPerSec += 5; // money per sec gains by 5
        ScoreCount.autoUpgrade1Cost *= 1.2; // autoUpgrade1Cost goes up in price  
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade1Cost.ToString()); // Text updates to new price
    }
    else {
...
function NotEnough () {
    ...
    textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade1Cost.ToString()); // text returns normal price
    isNotEnough = false;
}
