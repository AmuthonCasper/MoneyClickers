...
function Start () {
    /*
    if (PlayerPrefs.HasKey("SavedmPerSec")) { // Check if "SavedmPerSec" is in the system
        ScoreCount.mPerSec = PlayerPrefs.GetInt("SavedmPerSec"); // Putting value of key onto variable
    }
    else {
        PlayerPrefs.SetInt("SavedmPerSec", 0); // Set "SavedmPerSect" to standard 
        ScoreCount.mPerSec = PlayerPrefs.GetInt("SavedmPerSec"); // Putting value of key onto variable 
    }
    */
}
...
function Update () {
    //PlayerPrefs.SetInt("SavedmPerSec", ScoreCount.mPerSec); // Putting value into key, every frame (Always saving) // Calling it here because "PanelUpgrades" Only gets activated by button
    textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
    textField.text = "Per Sec: " + ToDot2(ScoreCount.mPerSec.ToString()); // Update mPerSec text 
}
