...
function Start () {
    /*
    if (PlayerPrefs.HasKey("SavedautoUpgrade2Cost")) { // Check if "SavedautoUpgrade2Cost" is in the system
        ScoreCount.autoUpgrade2Cost = PlayerPrefs.GetInt("SavedautoUpgrade2Cost"); // Putting value of key onto variable
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade2Cost.ToString()); // Update autoUpgrade2Cost text
    }
    else {                                                 // if "SavedautoUpgrade2Cost" isn't in the system
        PlayerPrefs.SetInt("SavedautoUpgrade2Cost", 15000);  // Set "SavedautoUpgrade2Cost" to standard (1000)
        ScoreCount.autoUpgrade2Cost = PlayerPrefs.GetInt("SavedautoUpgrade2Cost"); // Putting value of key onto variable
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade2Cost.ToString());  // Update autoUpgrade2Cost text
    }
...
function Update () {
    ...
    if (!isNotEnough) {
        textField = GetComponent(UI.Text); // Reference to UI.Text Componement (see 1 below)
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade2Cost.ToString());  // Update autoUpgrade2Cost text
        //PlayerPrefs.SetInt("SavedautoUpgrade2Cost", ScoreCount.autoUpgrade2Cost); // Putting value into key, every frame (Always saving)
    }
}

function BuyAutoUpgrade2 () {
    if (ScoreCount.money >= ScoreCount.autoUpgrade2Cost) { // If you have enough money
        canPlaySound = true;
        ScoreCount.money -= ScoreCount.autoUpgrade2Cost; // deduct money by the cost of the upgrade
        ScoreCount.mPerSec += 15; // money per sec gains by 15
        ScoreCount.autoUpgrade2Cost *= 1.5; // autoUpgrade2Cost goes up in price  
        textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade2Cost.ToString()); // Text updates to new price
    }
    else {
...
function NotEnough () {
    ...
    textField.text = "Buy For " + ToDot2(ScoreCount.autoUpgrade2Cost.ToString()); // text returns normal price
    isNotEnough = false;
}
